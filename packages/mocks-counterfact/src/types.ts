...
type IfHasKey<SomeObject, Keys extends (keyof any)[], Yes, No> = Keys extends [infer FirstKey, ...infer RestKeys]
...
type MaybeShortcut<ContentTypes extends MediaType[], Response extends OpenApiResponse> = IfHasKey<
  Response["content"],
  ContentTypes,
...
type MatchFunction<Response extends OpenApiResponse> = <ContentType extends MediaType & keyof Response["content"]>(
...
type HeaderFunction<Response extends OpenApiResponse> = <Header extends string & keyof Response["headers"]>(
  header: Header,
...
type GenericResponseBuilderInner<Response extends OpenApiResponse = OpenApiResponse> = OmitValueWhenNever<{
  header: [keyof Response["headers"]] extends [never] ? never : HeaderFunction<Response>;
  ...
  match: [keyof Response["content"]] extends [never] ? never : MatchFunction<Response>;
...
type GenericResponseBuilder<Response extends OpenApiResponse = OpenApiResponse> =
...
type ResponseBuilderFactory<Responses extends OpenApiResponses = OpenApiResponses> = {
  [StatusCode in keyof Responses]: GenericResponseBuilder<Responses[StatusCode]>;
